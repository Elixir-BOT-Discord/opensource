...
module.exports.run = async(client, message, args) => {
	if(!message.guild.member(message.author).roles.cache.has(config.dev)) return message.channel.send(':x: **Erreur** Seul les développeurs du bot peuvent utiliser la commande !')
		...
		user.roles.remove(config.hunter);
		message.channel.send(`<a:elixircheck:781228712590180392> ${user} n'est plus **Bug Hunter**`)
}

module.exports.help = {
	name: 'removehunter',
	aliases: ["Removehunter", "removeh", "rhunter", "delhunter", "rh"]
}